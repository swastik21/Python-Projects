...
record_db = mysql.connector.connect(host="localhost", user="root", passwd="Swastik21@", database="hotel")

mycursor = record_db.cursor()
...
def Booking(): 
		...
		if ch==0: 
			...
			ch=int(input("->")) 
		if ch==1: 
			room.append('Standard_Non-AC') 
			print("Room Type- Standard Non-AC") 
			price.append(3500) 
			print("Price- 3500") 
		elif ch==2: 
			room.append('Standard_AC') 
			print("Room Type- Standard AC") 
			price.append(4000) 
			print("Price- 4000") 
		elif ch==3: 
			room.append('3-Bed_Non-AC') 
			print("Room Type- 3-Bed Non-AC") 
			price.append(4500) 
			print("Price- 4500") 
		elif ch==4: 
			room.append('3-Bed_AC') 
...
def insertRec():
	...
	if phno!=[]: 
		global i
		for n in range(0,i):
			mycursor.execute(f"Insert into records(name,phone,addr,chkin,chkout,roomtype,price) values('{name[n]}',{phno[n]},'{add[n]}','{checkin[n]}','{checkout[n]}','{room[n]}',{price[n]})") 
		record_db.commit()
